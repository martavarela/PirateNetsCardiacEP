...
class DataSampler(BaseSampler):
    def __init__(self, coords, V, test_size, batch_size, train=True, rng_key=random.PRNGKey(1234)):
        ...
        coords_train, coords_test, V_train, _ = train_test_split(
            self.coords, self.V, test_size=test_size, random_state=42)
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    # Get dataset
    coords, t_star, x_star, V, W  = get_dataset("1DAPplanar.mat")
    V0 = V[0, :].reshape(-1, 1)
    ...
    # Initialize model
    model = models.AlievPanfilov1D(config, V0, V, t_star, x_star, coords)
