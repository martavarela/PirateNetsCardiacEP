...
class AlievPanfilov1D(ForwardIVP):
    def __init__(self, config, V0, V, t_star, x_star, coords):
        ...
        self.V_pred_fn = vmap(self.u_net, (None, 0, 0))
        self.W_pred_fn = vmap(self.W_net, (None, 0, 0))
        self.r_pred_fn = vmap(self.r_net, (None, 0, 0))

    def neural_net(self, params, t, x):
        tx = jnp.stack([t, x])
        print(f"Shape of x before FourierEmbs: {tx.shape}")
        _, outputs = self.state.apply_fn(params, tx)
        V = outputs[0]
        W = outputs[1]
        return V, W
    
    def u_net(self, params, t, x):
        V, _ = self.neural_net(params, t, x)
        return V

    def W_net(self, params, t, x):
        _, W = self.neural_net(params, t, x)
        return W
    ...
    def r_net(self, params, t, x):
        ...
        V, W = self.neural_net(params, t, x)
        V_t = grad(self.u_net, argnums=1)(params, t, x) # derivative of V with respect to t (2nd argument)
        W_t = grad(self.W_net, argnums=1)(params, t, x)
        V_hessian = hessian(self.u_net, argnums=(1, 2))(params, t, x)

        V_xx = V_hessian[1][1]

        eq_V = V_t - self.D*(V_xx) + self.k*V*(V-self.a)*(V-1) + W*V
        eq_W = W_t - (self.epsilon + (self.mu_1*W)/(self.mu_2+V))*(-W -self.k*V*(V-self.b-1))

        return eq_V, eq_W

    def rV_net(self, params, t, x):
        rV, _ = self.r_net(params, t, x)
        return rV
    
    def rW_net(self, params, t, x):
        _, rW = self.r_net(params, t, x)
        return rW
    ...
    def res_and_w(self, params, batch):
        ...
        rV_pred, rW_pred = \
            vmap(self.r_pred_fn, (None, 0, 0))(params, t_sorted, batch[:, 1])
    ...
    def losses(self, params, batch):
        ...
        V_pred_ic = vmap(self.u_net, (None, None, 0))(params, self.t0, self.x_star)
        # V_pred_ic = vmap(self.u_net, (None, None, 0))(params, 
        #     jnp.full_like(self.coords[0:100,1], self.t0), self.coords[0:100,1])

        print('The shape of V_pred_ic is:', V_pred_ic.shape)
        ics_loss = jnp.mean((self.V0 - V_pred_ic) ** 2)
    
        # No-flux boundary condition loss
        # du_dx_bound = grad(self.V_pred_fn, argnums=1)(params, t, x)
        # bc_loss = jnp.mean( (du_dx_bound - 0) ** 2 )

        V_pred_data = self.V_pred_fn(params, data_coords_batch[:, 0], data_coords_batch[:, 1])
        print('V_pred_data shape:', V_pred_data.shape)
        ...
        V_loss = jnp.mean((V_train - V_pred_data) ** 2)
        ...
        if self.config.weighting.use_causal == True:
            ...
            rV_pred, rW_pred = self.r_pred_fn(params, batch[:, 0], batch[:, 1])
            res_loss = jnp.mean((rV_pred) ** 2 + (rW_pred) ** 2)
        
        loss_dict = {"ics": ics_loss, "res": res_loss, "data": V_loss}
        # loss_dict = {"ics": ics_loss, "data": V_loss}
        return loss_dict
    ...
    def compute_l2_error(self, params, V_test):
        V_pred = self.V_pred_fn(params, self.coords[:,0], self.coords[:,1])
...
class Evaluator(BaseEvaluator):
    ...
    def log_preds(self, params):
        u_pred = self.model.V_pred_fn(params, self.model.t_star, self.model.x_star, self.model.y_star)
